...
def test_normalform():
    ...
    signals.append(
        sigAdd(
            sigAdd(sigDelay(sigDelay(sigInput(0), sigReal(500)), sigReal(200)), sigReal(0.5)),
            sigReal(3),
        )
    )
    ...
    # Compute normal form
    simplifyToNormalForm(signals[0])
    simplifyToNormalForm2(signals)
...
def test8():
    ...
    slider = sigVSlider("Vol", sigReal(0.5), sigReal(0.0), sigReal(1.0), sigReal(0.01))
...
def test9():
    ...
    freq = sigVSlider("h:Oscillator/freq", sigReal(440), sigReal(50), sigReal(1000.0), sigReal(0.1))
    gain = sigVSlider("h:Oscillator/gain", sigReal(0), sigReal(0), sigReal(1), sigReal(0.011))
...
def test12():
    ...
    signals += sigWaveform([i * 100.0 for i in range(5)])
...
def test22():
    ...
    signals.append(
        osc(
            f,
            sigVSlider(
                "h:Oscillator/Freq1", sigReal(300), sigReal(100), sigReal(2000), sigReal(0.01)
            ),
        )
    )
    signals.append(
        osc(
            f,
            sigVSlider(
                "h:Oscillator/Freq2", sigReal(500), sigReal(100), sigReal(2000), sigReal(0.01)
            ),
        )
    )
...
def test24():
    ...
    organ = sigMul(
        gate, sigAdd(sigMul(osc(f, freq), gain), sigMul(osc(f, sigMul(freq, sigInt(2))), gain))
    )
    organ *= 0.2
    signals = [organ, organ]
