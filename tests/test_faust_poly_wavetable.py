...
def test_faust_poly_wavetable_sine():
    ...
    waveform_length = 4
    wavecycle = np.sin(np.pi * 2.0 * np.arange(waveform_length) / float(waveform_length))
...
def test_faust_poly_wavetable_saw():
    waveform_length = 4
    wavecycle = -1.0 + 2.0 * np.arange(waveform_length) / float(waveform_length)
