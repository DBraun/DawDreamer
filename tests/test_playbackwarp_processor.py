...
def test_playbackwarp_processor1(buffer_size: int):
    ...
    bpm = np.concatenate([140 * np.ones(full_measure), 70.0 * np.ones(full_measure)])
    bpm = np.tile(bpm, (100))
    engine.set_bpm(bpm, ppqn=ppqn)

    drums = engine.make_playbackwarp_processor(
    ...
    other = engine.make_playbackwarp_processor(
    ...
    drums.set_options(rb_option.OptionTransientsMixed | rb_option.OptionChannelsApart)

    graph = [
        (drums, []),
        (other, []),
        (engine.make_add_processor("add", [1.0, 1.0]), ["drums", "other"]),
    ]

    engine.load_graph(graph)
    render(engine, file_path=OUTPUT / "test_playbackwarp_processor1a.wav", duration=DURATION)

    assert np.mean(np.abs(engine.get_audio())) > 0.01
    ...
    other.set_automation("transpose", make_sine(1.0, DURATION))
...
def test_playbackwarp_processor2(buffer_size: int):
    ...
    drums.set_clip_positions([[0.0, 4.0, 0.0], [5.0, 9.0, 0.0]])

    render(engine, file_path=OUTPUT / "test_playbackwarp_processor2a.wav")

    assert np.mean(np.abs(engine.get_audio())) > 0.01

    drums.set_clip_positions([[0.0, 4.0, 0.0]])
    drums.start_marker = 0.0
    ...
    drums.start_marker = 0.0
    drums.loop_start = 3.0
    drums.loop_end = 4.0
    drums.set_clip_positions([[0.0, 2.0, 0.0], [3.0, 9.0, 3.0]])
...
def test_playbackwarp_processor3(loop_end: float):
    ...
    drum_audio = drum_audio[:, int(SAMPLE_RATE * 0.267) :]  # manually trim to beginning

    drums = engine.make_playbackwarp_processor("drums", drum_audio)

    drums.reset_warp_markers(108)
    drums.set_clip_positions([[0.0, i, 0.0] for i in range(1, 4)])

    drums.time_ratio = 0.7
    assert drums.time_ratio == 0.7
    ...
    render(
        engine,
        file_path=OUTPUT / f"test_playbackwarp_processor3a_loop{loop_end}.wav",
        duration=DURATION,
    )
    ...
    assert np.mean(np.abs(audio)) > 0.02
