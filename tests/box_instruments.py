...
def semiToRatio(box: Box) -> Box:
    return boxSeq(box, boxPow(boxReal(2.0), boxWire() / 12.0))
...
def phasor(freq: Box) -> Box:
    return boxSeq(
        boxDiv(freq, boxSampleRate()), boxRec(boxSplit(boxAdd(), decimalpart()), boxWire())
    )
...
class ModularSynth:
    ...
    def _process_modulations(self, modulations):
        for source, dst, amt, symmetric in modulations:
            if source not in self._MODS:
                warnings.warn(
                    f"""warning: source "{source}" was modulated but isn't used for DSP.""",
                    stacklevel=2,
                )
                continue
            if dst not in self._MODS:
                warnings.warn(
                    f"""warning: destination "{dst}" was modulated but isn't used for DSP.""",
                    stacklevel=2,
                )
                continue

            if symmetric:
                self._MODS[dst] += (self._MODS[source] - 0.5) * amt * 2.0
            else:
                self._MODS[dst] += self._MODS[source] * amt

    def _make_macro(self, i: int):
        self._MODS[f"macro{i}"] = boxHSlider(f"h:Macro/Macro {i}", 0, 0, 1, 0.001)

    def _make_env(self, i: int):
        # time units are milliseconds
        self._MODS[f"env{i}_A"] = (
            boxWire() + boxHSlider(f"h:Env {i}/[0]Attack", 5, 0.0, 10_000, 0.001)
        ) / 1_000
        self._MODS[f"env{i}_H"] = (
            boxWire() + boxHSlider(f"h:Env {i}/[1]Hold", 0, 0.0, 10_000, 0.001)
        ) / 1_000
        self._MODS[f"env{i}_D"] = (
            boxWire() + boxHSlider(f"h:Env {i}/[2]Decay", 20, 0.0, 10_000, 0.001)
        ) / 1_000
        self._MODS[f"env{i}_S"] = boxWire() + boxHSlider(
            f"h:Env {i}/[3]Sustain", 0.5, 0.0, 10_000, 0.001
        )
        self._MODS[f"env{i}_R"] = (
            boxWire() + boxHSlider(f"h:Env {i}/[4]Release", 200, 0.0, 10_000, 0.001)
        ) / 1_000
        self._MODS[f"env{i}"] = boxFromDSP("process = en.ahdsre;")

    def _make_lfo(self, i: int, trigger_choice):
        # todo: use the trigger_choice

        self._MODS[f"lfo{i}_gain"] = boxMax(
            boxReal(0), boxWire() + boxHSlider(f"h:LFO {i}/[0]Gain", 1, 0, 10, 0.001)
        )
        self._MODS[f"lfo{i}_freq"] = boxWire() + boxHSlider(f"h:LFO {i}/[1]Freq", 2, 0, 10, 0.001)

        self._MODS[f"lfo{i}"] = boxFromDSP("""process(gain, freq, gate) = gain * os.osc(freq);""")
    ...
    def _get_wavecycle_data(self, choice) -> list[float]:
        ...
        if choice == OscChoice.SINE:
            ...
            wavecycle_data = -1.0 + 2.0 * np.linspace(0, 1, TABLE_SIZE, endpoint=False)
            ...
            t = np.linspace(0, 1, TABLE_SIZE, endpoint=False)
            wavecycle_data = -1.0 + 2.0 * (t > 0.5).astype(np.float32)
        elif choice == OscChoice.WHITE_NOISE:
            wavecycle_data = -1.0 + 2.0 * np.random.rand(TABLE_SIZE)
        else:
    ...
    def _make_noise(self):
        ...
        self._MODS["noise_gain"] = boxMax(
            boxReal(0), boxWire() + boxHSlider("h:Noise/[0]Gain", 0, 0, 10.0, 0.001)
        )
        ...
        readTable = boxWire() * boxReadOnlyTable(
            boxInt(TABLE_SIZE),
            waveform_content,
            phasor(f, TABLE_SIZE / boxSampleRate()) * TABLE_SIZE,
        )

        self._MODS["noise"] = boxSplit(readTable, bus(2))  # split to stereo

    def _make_sub(self, choice):
        self._MODS["sub_gain"] = boxMax(
            boxReal(0), boxWire() + boxHSlider("h:Sub/[0]Gain", 0, 0, 10, 0.001)
        )
        self._MODS["sub_freq"] = (
            semiToRatio(boxWire() + boxHSlider("h:Sub/[1]Freq", 0, -72, 72, 0.001))
            * self._MODS["freq"]
        )
        self._MODS["sub_pan"] = boxWire() + boxHSlider("h:Sub/[2]Pan", 0.5, 0, 1, 0.001)
        self._MODS["sub_waveform"] = boxWaveform(self._get_wavecycle_data(choice))
        ...
        """
        self._MODS["sub"] = boxFromDSP(dsp_code)

    def _make_osc(self, x: str, choice, unison: int):
        self._MODS[f"osc{x}_gain"] = boxMax(
            boxReal(0), boxWire() + boxHSlider(f"h:Osc {x}/[0]Gain", 0, 0, 10, 0.001)
        )
        self._MODS[f"osc{x}_freq"] = (
            semiToRatio(boxWire() + boxHSlider(f"Osc {x} [1]Freq", 0, 0, 10, 0.001))
            * self._MODS["freq"]
        )
        self._MODS[f"osc{x}_detune_amt"] = boxWire() + boxHSlider(
            f"h:Osc {x}/[2]Detune", 0.5, 0, 10, 0.001
        )
        self._MODS[f"osc{x}_blend"] = boxWire() + boxHSlider(
            f"h:Osc {x}/[3]Blend", 0.5, 0, 10, 0.001
        )
        self._MODS[f"osc{x}_pan"] = (
            boxWire() + boxHSlider(f"h:Osc {x}/[4]Pan", 0.5, 0, 1, 0.001)
        ) * 2.0 - 1
        self._MODS[f"osc{x}_wt_pos"] = boxWire() + boxHSlider(
            f"h:Osc {x}/[5]WT Pos", 0.0, 0, 1, 0.001
        )
        self._MODS[f"osc{x}_wt_bend"] = boxWire() + boxHSlider(
            f"h:Osc {x}/[6]WT Bend", 0.0, 0, 1, 0.001
        )
        self._MODS[f"osc{x}_phase"] = boxWire() + boxHSlider(f"h:Osc {x}/[7]Phase", 0, 0, 1, 0.001)
        self._MODS[f"osc{x}_rand"] = boxWire() + boxHSlider(f"h:Osc {x}/[8]Rand", 0, 0, 1, 0.001)
        self._MODS[f"osc{x}_stereo_width"] = boxWire() + boxHSlider(
            f"h:Osc {x}/[9]Stereo Width", 1, 0, 1, 0.001
        )
    ...
    def _make_filter(self, choice):
        self._MODS["filter_cutoff"] = boxWire() + boxHSlider(
            "h:Filter/Cutoff", 5000.0, 20.0, 20000, 0.001
        )
        self._MODS["filter_gain"] = boxWire() + boxHSlider(
            "h:Filter/Gain", 0.0, -80, 24, 0.001
        )  # todo: dB to gain
        self._MODS["filter_resonance"] = boxWire() + boxHSlider(
            "h:Filter/Resonance", 1, 0, 2, 0.001
        )
    ...
    def _make_reverb(self):
        self._MODS["reverb_cutoff"] = boxWire() + boxHSlider(
            "h:Reverb/Filter Cutoff", 5000.0, 20.0, 20000, 0.001
        )
        self._MODS["reverb_size"] = boxWire() + boxHSlider("h:Reverb/Size", 0, 0, 1, 0.001)
        self._MODS["reverb_mix"] = boxWire() + boxHSlider("h:Reverb/Mix", 0, 0, 1, 0.001)
    ...
    def _make_delay(self):
        ...
        self._MODS["delay_dtime"] = boxHSlider("h:Delay/Time", 0.125, 0.0, MAXDELAY, 0)
        ...
        self._MODS["delay_wet"] = boxHSlider("h:Delay/Wet", 0.5, 0, 1, 0)
    ...
    def _parse_modulations(all_modulations):
        ...
        for modulation in all_modulations:
            source, _dst = modulation[0].lower(), modulation[1].lower()
            if (
                source.startswith("macro")
                or source.startswith("gain")
                or source.startswith("gate")
                or source.startswith("freq")
            ):
    ...
    def build(self, cfg: dict) -> Box:
        MACRO_MODULATIONS, OTHER_MODULATIONS, EFFECTS_MODULATIONS = self._parse_modulations(
            cfg["MODULATION_MATRIX"]
        )
        ...
        self._MODS = {
            "freq": boxHSlider("freq", 100, 100, 20000, 0.001),
            "gate": boxButton("gate"),
            "gain": boxHSlider("gain", 0.5, 0, 1, 0.001),
        }
        ...
        for name, osc_toggle, filter_osc_toggle in [
            ("oscA", OSC_A_TOGGLE, FILTER_OSC_A),
            ("oscB", OSC_B_TOGGLE, FILTER_OSC_B),
        ]:
            if osc_toggle:
                self._MODS[name] = boxSplit(
                    boxParN(
                        [
                            self._MODS[f"{name}_waveform"],
                            self._MODS[f"{name}_gain"],
                            self._MODS[f"{name}_stereo_width"],
                            self._MODS[f"{name}_detune_amt"],
                            self._MODS[f"{name}_blend"],
                            self._MODS[f"{name}_freq"],
                            self._MODS[f"{name}_pan"],
                            self._MODS["gate"],
                        ]
                    ),
                    self._MODS[name],
        ...
        if FILTER_TOGGLE:
            to_filter = boxSeq(
                boxPar4(
                    self._MODS["filter_cutoff"],
                    self._MODS["filter_gain"],
                    self._MODS["filter_resonance"],
                    to_filter,
                ),
                self._MODS["filter"],
            )
        ...
        instrument = boxSplit(boxReal(0.0), after_filter)

        self._process_modulations(EFFECTS_MODULATIONS)

        for effect in EFFECTS:
            if effect == "reverb" or effect == "chorus":
                pass
            elif effect == "delay":
                instrument = boxSeq(
                    boxParN(
                        [
                            self._MODS["delay_dtime"],
                            self._MODS["delay_level"],
                            self._MODS["delay_feedback"],
                            self._MODS["delay_stereo"],
                            self._MODS["delay_wet"],
                            instrument,
                        ]
                    ),
                    self._MODS["delay"],
