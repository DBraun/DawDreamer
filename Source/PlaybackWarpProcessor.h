...
class PlaybackWarpProcessor : public ProcessorBase {
 public:
  ...
  void setStartMarker(double startMarker) {
    m_clipInfo.start_marker = startMarker;
  }
  ...
  class Clip {
   public:
    Clip(double startPos, double endPos, double startMarkerOffset)
        : start_pos{startPos},
          end_pos{endPos},
          start_marker_offset{startMarkerOffset} {};
    Clip()
        : start_pos{0.},
          end_pos{std::numeric_limits<double>::max()},
