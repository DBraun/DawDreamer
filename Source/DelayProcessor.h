...
class DelayProcessor : public ProcessorBase {
 public:
  DelayProcessor(std::string newUniqueName, std::string rule, float delaySize,
                 float wet)
  ...
  void processBlock(juce::AudioSampleBuffer& buffer,
    ...
    for (int chan = 0; chan < buffer.getNumChannels(); chan++) {
      buffer.addFrom(chan, 0, delayBuffer, chan, 0, buffer.getNumSamples(),
                     *myWetLevel);
    }
 ...
 private:
  ...
  std::atomic<float>*
      myWetLevel;  // 0 means use all of original signal and none of the "wet"
                   // delayed signal. 1. is the opposite.
  std::string
      myRule;  // todo: ugly because storing an enum would probably be better.
