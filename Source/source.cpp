...
Unlike a VST, the parameters don't need to be between 0 and 1. For example, you can set an envelope attack parameter to 50 to represent 50 milliseconds.";
    ...
    nb::class_<DawDreamerFaustLibContext>(
        ...
        .def("__exit__", [](DawDreamerFaustLibContext& self, nb::args args)
             { self.exit(args[0], args[1], args[2]); });

    auto box_module = faust.def_submodule("box");
    create_bindings_for_faust_box(faust, box_module);

    auto signal_module = faust.def_submodule("signal");
    create_bindings_for_faust_signal(faust, signal_module);
