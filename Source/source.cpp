...
play the audio in double the amount of time, so it will sound slowed down.")
        ...
        .def_prop_rw("end_marker", &PlaybackWarpProcessor::getEndMarker,
                     ...
                     "relative to 1.1.1")
        .def_prop_ro("warp_markers", &PlaybackWarpProcessor::getWarpMarkers, nb::rv_policy::move,
                     "Get the warp markers as an (N, 2) numpy array of time "
                     "positions in samples and positions in beats.")
        .def("set_warp_markers", &PlaybackWarpProcessor::setWarpMarkers, arg("markers"),
             "Set the warp markers as an (N, 2) numpy array of time "
             "positions in samples and positions in beats.")
        ...
        .def("set_data", &PlaybackWarpProcessor::setData, arg("data"), kw_only(), arg("sr") = 0,
             "Set the audio as a numpy array shaped (Channels, Samples) with an "
             "optional `sr` kwarg for the sample rate of the data.")
        .def("set_options", &PlaybackWarpProcessor::setRubberBandOptions, arg("config"),
        ...
        .def("set_clip_positions", &PlaybackWarpProcessor::setClipPositions, arg("clip_positions"),
             R"pbdoc(
...
#include "rubberband/rubberband/RubberBandStretcher.h"
    ...
    nb::enum_<RubberBandStretcher::Option>(playbackWarpProcessor, "option", nb::is_arithmetic())
        ...
        .def("__xor__", [](RubberBandStretcher::Option e1, RubberBandStretcher::Option other)
             { return e1 | other; });
...
but the filter mode cannot under automation.";
    ...
    nb::class_<PluginProcessorWrapper, ProcessorBase>(m, "PluginProcessor")
        .def("can_set_bus", &PluginProcessorWrapper::canApplyBusInputsAndOutputs, arg("inputs"),
             ...
             "channels can be set.")
        .def("set_bus", &PluginProcessorWrapper::setMainBusInputsAndOutputs, arg("inputs"),
             arg("outputs"),
        ...
        .def("get_parameter", &PluginProcessorWrapper::getAutomationAtZeroByIndex, arg("index"),
             "Get a parameter's value.")
        .def("get_parameter_name", &PluginProcessorWrapper::wrapperGetParameterName, arg("index"),
             "Get a parameter's name.")
        .def("get_parameter_text", &PluginProcessorWrapper::getParameterAsText, arg("index"),
             "Get a parameter's value as text.")
        .def("set_parameter", &PluginProcessorWrapper::wrapperSetParameter, arg("index"),
             arg("value"), "Set a parameter's value to a constant.")
        ...
        .def("get_parameter_range", &PluginProcessorWrapper::getParameterValueRange, arg("index"),
...
Note that note-ons and note-offs are counted separately.")
        ...
        .def("add_midi_note", &PluginProcessorWrapper::addMidiNote, arg("note"), arg("velocity"),
...
or effects. Some plugins such as ones that do sidechain compression can accept two inputs when loading a graph.";
    ...
    nb::class_<SamplerProcessor, ProcessorBase>(m, "SamplerProcessor")
        ...
        .def("get_parameter_name", &SamplerProcessor::wrapperGetParameterName, arg("index"),
             "Get a parameter's name.")
        .def("get_parameter_text", &SamplerProcessor::wrapperGetParameterAsText, arg("index"),
             "Get a parameter's value as text.")
        .def("set_parameter", &SamplerProcessor::setAutomationValByIndex, arg("index"),
             arg("value"), "Set a parameter's value to a constant.")
...
Note that note-ons and note-offs are counted separately.")
        ...
        .def("add_midi_note", &SamplerProcessor::addMidiNote, arg("note"), arg("velocity"),
             arg("start_time"), arg("duration"), kw_only(), arg("beats") = false,
             add_midi_description)
...
Unlike a VST, the parameters don't need to be between 0 and 1. For example, you can set an envelope attack parameter to 50 to represent 50 milliseconds.";
    ...
    nb::class_<RenderEngine>(m, "RenderEngine",
        ...
        .def("make_sampler_processor", &RenderEngine::makeSamplerProcessor, arg("name"),
             ...
             returnPolicy)
#ifdef BUILD_DAWDREAMER_FAUST
        .def("make_faust_processor", &RenderEngine::makeFaustProcessor, arg("name"),
             "Make a FAUST Processor", returnPolicy)
#endif
        .def("make_playback_processor", &RenderEngine::makePlaybackProcessor, arg("name"),
             arg("data"), returnPolicy, "Make a Playback Processor")
#ifdef BUILD_DAWDREAMER_RUBBERBAND
        .def("make_playbackwarp_processor", &RenderEngine::makePlaybackWarpProcessor, arg("name"),
             arg("data"), kw_only(), arg("sr") = 0,
             ...
             returnPolicy)
#endif
        .def("make_filter_processor", &RenderEngine::makeFilterProcessor, returnPolicy, arg("name"),
             arg("mode") = "high", arg("freq") = 1000.f, arg("q") = .707107f, arg("gain") = 1.f,
             "Make a Filter Processor")
        .def("make_reverb_processor", &RenderEngine::makeReverbProcessor, returnPolicy, arg("name"),
             arg("roomSize") = 0.5f, arg("damping") = 0.5f, arg("wetLevel") = 0.33f,
             arg("dryLevel") = 0.4f, arg("width") = 1.0f, "Make a Reverb Processor")
        .def("make_add_processor", &RenderEngine::makeAddProcessor, returnPolicy, arg("name"),
             arg("gain_levels") = defaultGain,
             "Make an Add Processor with an optional list of gain levels")
        .def("make_delay_processor", &RenderEngine::makeDelayProcessor, returnPolicy, arg("name"),
             arg("rule") = "linear", arg("delay") = 10.f, arg("wet") = .1f,
             "Make a Delay Processor")
        .def("make_panner_processor", &RenderEngine::makePannerProcessor, returnPolicy, arg("name"),
