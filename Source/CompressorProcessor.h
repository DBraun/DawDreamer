...
class CompressorProcessor : public ProcessorBase {
 public:
  CompressorProcessor(std::string newUniqueName, float threshold, float ratio,
  ...
  void setThreshold(float threshold) {
    setAutomationVal("threshold", threshold);
  }
 ...
 public:
  void createParameterLayout() {
    ...
    group.addChild(std::make_unique<AutomateParameterFloat>(
        "ratio", "ratio",
        NormalisableRange<float>(1.f, std::numeric_limits<float>::max()), 0.f));
    group.addChild(std::make_unique<AutomateParameterFloat>(
