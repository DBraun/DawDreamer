...
class AbletonClipInfo
{
  public:
    ...
    AbletonClipInfo()
        : loop_start(0), loop_end(std::numeric_limits<double>::infinity()), start_marker(0),
    ...
    bool readWarpFile(const char* path)
    {
        ...
        {
            if (!fseek(f, 4, SEEK_CUR) && read_double(f, &pos) && read_double(f, &beat))
            {
                found_one = true;
  ...
  private:
    ...
    int read_loop_info(FILE* f)
    {
        ...
        if (find_str(f, "SampleOverViewLevel") && find_str(f, "SampleOverViewLevel") &&
            !fseek(f, 71, SEEK_CUR) && read_double(f, &loop_start) && read_double(f, &loop_end) &&
        ...
        else
        {
            ...
            if (find_str(f, "SampleData") && find_str(f, "SampleData") &&
                ...
                read_double(f, &hidden_loop_start) && read_double(f, &hidden_loop_end) &&
                read_double(f, &end_marker) && !fseek(f, 3, SEEK_CUR) && read_bool(f, &warp_on))
            {
                start_marker = loop_start + sample_offset;
                return 1;
            }
