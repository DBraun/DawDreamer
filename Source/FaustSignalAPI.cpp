  ...
  signal_module
      ...
      .def("box_test1",
           []() {
             ...
             int inputs, outputs;
             std::string error_msg;
             Box box = DSPToBoxes("test", "process = 0.5 : cos ;", 0, nullptr,
                                  &inputs, &outputs, error_msg);
             if (!error_msg.empty()) {
               throw std::runtime_error("dsp to boxes error " + error_msg);
             }
             std::string code = "";
             code += "box code:\n" + printBox(box, true, 100000) + "\n";

             tvec signals = boxesToSignals(box, error_msg);
             if (!error_msg.empty()) {
               throw std::runtime_error("boxesToSignals error " + error_msg);
             }

             if (signals.empty()) {
               throw std::runtime_error("signals is empty");
             }
             code += "signal code:\n" + printSignal(signals.at(0), true, 1000) +
                     "\n";
             destroyLibContext();
             return code;
           })
      ...
      .def("box_test2",
           []() {
             // this fails at printBox. I think it's because box3 was composed
             // "manually" instead of from DSPtoBoxes.
             createLibContext();
      ...
      .def("sig_test1",
           []() {
             // everything works
             createLibContext();
             Signal s1 = sigReal(0.5);
             Signal s2 = sigCos(s1);
             destroyLibContext();
           })
      .def("sig_test2",
           []() {
             // this fails at printSignal
             createLibContext();
             Signal s1 = sigReal(0.5);
             Signal s2 = sigCos(s1);
             std::string code = printSignal(s2, true, 10000);
             destroyLibContext();
             return code;
           })
      .def(
