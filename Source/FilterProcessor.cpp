...
FilterProcessor::FilterProcessor(std::string newUniqueName, std::string mode, float freq = 1000.f,
                                 float q = 0.707107f, float gain = 1.f)
...
void FilterProcessor::prepareToPlay(double sampleRate, int samplesPerBlock)
{
    ...
    int numChannels = 2;
    juce::dsp::ProcessSpec spec{mySampleRate, static_cast<juce::uint32>(mySamplesPerBlock),
...
void FilterProcessor::automateParameters(AudioPlayHead::PositionInfo& posInfo, int numSamples)
{
    ...
    case FILTER_FilterFormat::LOW_PASS:
        *myFilter.state =
            *juce::dsp::IIR::Coefficients<float>::makeLowPass(mySampleRate, myFreq, myQ);
        break;
    case FILTER_FilterFormat::BAND_PASS:
        *myFilter.state =
            *juce::dsp::IIR::Coefficients<float>::makeBandPass(mySampleRate, myFreq, myQ);
        break;
    case FILTER_FilterFormat::HIGH_PASS:
        *myFilter.state =
            *juce::dsp::IIR::Coefficients<float>::makeHighPass(mySampleRate, myFreq, myQ);
        break;
    case FILTER_FilterFormat::LOW_SHELF:
        *myFilter.state =
            *juce::dsp::IIR::Coefficients<float>::makeLowShelf(mySampleRate, myFreq, myQ, myGain);
        break;
    case FILTER_FilterFormat::HIGH_SHELF:
        *myFilter.state =
            *juce::dsp::IIR::Coefficients<float>::makeHighShelf(mySampleRate, myFreq, myQ, myGain);
        break;
    case FILTER_FilterFormat::NOTCH:
        *myFilter.state =
            *juce::dsp::IIR::Coefficients<float>::makeNotch(mySampleRate, myFreq, myQ);
        break;
    default:
        return; // todo: throw error
        break;
...
std::string FilterProcessor::getMode()
{
...
void FilterProcessor::setGain(float gain)
{
