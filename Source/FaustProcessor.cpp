...
FaustProcessor::FaustProcessor(std::string newUniqueName, double sampleRate, int samplesPerBlock)
    : ProcessorBase{newUniqueName}
{
    ...
    // auto import
    m_autoImport = "// FaustProcessor (DawDreamer) auto "
                   "import:\nimport(\"stdfaust.lib\");\n";
}
...
bool FaustProcessor::canApplyBusesLayout(const juce::AudioProcessor::BusesLayout& layout)
{
...
void FaustProcessor::processBlock(juce::AudioSampleBuffer& buffer, juce::MidiBuffer& midiBuffer)
{
    ...
    if (m_compileState == kMono || m_compileState == kSignalMono)
    {
        if (m_dsp != NULL)
        {
            m_dsp->compute(buffer.getNumSamples(), (float**)buffer.getArrayOfReadPointers(),
...
void FaustProcessor::automateParameters(AudioPlayHead::PositionInfo& posInfo, int numSamples)
{
    if (!m_ui)
    {
        return;
...
void FaustProcessor::reset()
{
...

    {
        ...
        for (auto* parameter : getParameters())
        {
            ...
            m_ui->setParamValue(faustIndex, ((AutomateParameterFloat*)parameter)->sample(posInfo));
            i++;
        }
...
int FaustProcessor::getNumVoices()
{
    return m_nvoices;
}
...
int FaustProcessor::getGroupVoices()
{
...
int FaustProcessor::getDynamicVoices()
{
...
bool FaustProcessor::compile()
{
    ...
    {
        throw std::runtime_error("FaustProcessor::compile(): Error for path for faust libraries: " +
    ...
    if (!m_errorString.empty())
    {
        // output error
        clear();
        throw std::runtime_error("FaustProcessor::compile(): " + m_errorString +
...
std::string FaustProcessor::code()
{
    return m_code;
}
...
void FaustProcessor::createParameterLayout()
{
    ...
    for (int i = 0; i < m_ui->getParamsCount(); ++i)
    {
        ...
        if (m_nvoices > 0)
        {
            ...
            if (!m_groupVoices &&
                hasStart(parnameString, std::string("/Sequencer/DSP1/Polyphonic/Voices/")))
            {
...
nb::list FaustProcessor::getPluginParametersDescription()
{
    ...
    if (m_compileState)
    {
        ...
        for (auto* parameter : getParameters())
        {
            std::string theName = parameter->getName(DAW_PARAMETER_MAX_NAME_LENGTH).toStdString();
            ...
            long long numSteps =
                step <= std::numeric_limits<float>::min()
                    ? std::numeric_limits<long long>::max()
                    : (long long)((m_ui->getParamMax(faustIndex) - m_ui->getParamMin(faustIndex)) /
                                  step) +
                          1;
...
bool FaustProcessor::loadMidi(const std::string& path, bool clearPrevious, bool isBeats,
                              bool allEvents)
{
    if (!std::filesystem::exists(path.c_str()))
    {
...
double FaustProcessor::getReleaseLength()
{
