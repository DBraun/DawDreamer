...
void add_operation(py::class_<BoxWrapper> &cls, const char *name,
                   Box (*func)(Box, Box)) {
  cls.def(name,
          [func](const BoxWrapper &box, int other) {
            return BoxWrapper(func((BoxWrapper &)box, boxInt(other)));
          })
      .def(name,
           [func](const BoxWrapper &box, float other) {
...
void add_unary_operation(py::class_<BoxWrapper> &cls, const char *name,
                         Box (*func)(Box)) {
  cls.def(name, [func](const BoxWrapper &box1) {
    return BoxWrapper(func((BoxWrapper &)box1));
  });
}
...
py::module_ &create_bindings_for_faust_box(py::module &faust_module,
  ...
  add_operation(cls, "__lshift__",
                static_cast<Box (*)(Box, Box)>(boxLeftShift));
  add_operation(cls, "__rshift__",
                static_cast<Box (*)(Box, Box)>(boxARightShift));
  ...
  auto floordiv_func = [](Box box1, Box other) {
    return boxFloor(boxDiv(box1, other));
  };

  add_operation(cls, "__floordiv__",
                static_cast<Box (*)(Box, Box)>(floordiv_func));
  ...
  add_unary_operation(cls, "__float__",
                      static_cast<Box (*)(Box)>(boxFloatCast));
  ...
  box_module
      ...
      .def(
          ...
          arg("selector") = py::none(), arg("box1") = py::none(),
          arg("box2") = py::none(), arg("box3") = py::none())
