...
def keyframe(start: float, stop: float, beats: float, smooth=True):
    def smoothclamp(x, mi, mx):
        return mi + (mx - mi) * (
            lambda t: np.where(t < 0, 0, np.where(t <= 1, 3 * t**2 - 2 * t**3, 1))
        )((x - mi) / (mx - mi))

    y = np.linspace(0, 1, num=int(PPQN * beats), endpoint=True)
    if smooth:
        y = smoothclamp(y, 0.0, 1.0)  # any easing function can go here
...
class MyEngine(daw.RenderEngine):
    ...
    def load_audio(self, i: int, audio_path: str, bpm=120.0, start_sec=0.0):
        """
        ...
        playback = self.playback1 if i == 0 else self.playback2
        ...
        if isfile(asd_path):
            ...
            beats_in_one_sec_audio = bpm / 60.0
            playback.warp_markers = np.array(
                [[start_sec, 0.0], [start_sec + 1.0, beats_in_one_sec_audio]], dtype=np.float32
            )

    def render(self, duration: float, file_path=None):
        super().render(duration, beats=True)
